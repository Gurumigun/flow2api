...
test('bounded raster data URLs are accepted without admitting active image formats',()=>{
 const inline='data:image/png;base64,eA==';
 assert(allowed(inline));
 const parsed=parser(false)(inline);
 assert.equal(parsed.url,inline);
 assert.match(parsed.identity,/^data:/);
});
